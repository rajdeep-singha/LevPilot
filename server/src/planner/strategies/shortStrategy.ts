 ...
 *
 * Simulates a leveraged SHORT by executing only the DeepBook swap leg:
 *   User's SUI (capital) → DBUSDC via DeepBook SUI/USDC pool
 *
 * Production flow would be:
 *   1. Deposit USDC collateral → Scallop
...
export async function buildShortPTB(
  ...
  // Cap to available balance minus gas reserve
  const client = getSuiClient();
  const balanceData = await client.getBalance({ owner: walletAddress });
  const availableUnits = BigInt(balanceData.totalBalance) - GAS_BUDGET_MIST - 10_000_000n;
  ...
  // Swap SUI → DBUSDC on DeepBook (isBid=false: selling SUI for USDC)
